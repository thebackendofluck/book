...
from sentry_sdk.integrations.logging import LoggingIntegration  # ty:ignore[unresolved-import]
import numpy as np

import loadsim  # ty:ignore[unresolved-import]
from loadkit import DynamoClient  # ty:ignore[unresolved-import]
...
KAMBI_OFFERING_URL = os.getenv(
    "KAMBI_OFFERING_URL", "https://offering.example.com"
)
KAMBI_PLAYER_URL = os.getenv("KAMBI_PLAYER_URL", "https://auth.example.com")
KAMBI_OFFERING_ID = os.getenv("KAMBI_OFFERING_ID", "pivuspa")
WH_BASE_URL = os.getenv(
    "WH_BASE_URL", "https://platform.staging.casino-platform.example.com/api"
)
...
for bettor_type in list(bettors.keys()):
    with open(f"../config/{bettor_type}.yaml", "r") as f:
...
class RoboTasks(TaskSet):
    ...
    def on_start(self):
        ...
        if len(users_list) > 0 and prereg:
            ...
            self.tasks.append(lambda: self.login(None, 0))
        else:
            self.tasks.append(lambda: self.register(self.deckard.user_data, 0))
            self.prereg = False
    ...
    def register(self, data, wait):
        ...
        with self.client.post(
            ...
            self.kambi_session_id = resp_data.get("kambi_session_id")
            self.kambi_routing_key = resp_data.get("kambi_routing_key")
    ...
    def login(self, _, wait):
        with self.client.post(
            ...
            self.kambi_session_id = response_data["kambi_session_id"]
    ...
    def get_homepage(self, _, wait):
        ...
        self.client.get(
            f"{KAMBI_OFFERING_URL}/offering/v2018/{KAMBI_OFFERING_ID}/section/eventgroup/{groupids}.json"
            f"?lang=en_US&market=US&useCombined=true&onlyMatches=true",
            headers={"X-Corr-Id": self.user_correlation_id},
            name=f"Homepage<kambi>/offerings/section/eventgroup",
        )

        # call Kambi ListView with organized path data
        response = self.client.get(
            f"{KAMBI_OFFERING_URL}/offering/v2018/{KAMBI_OFFERING_ID}/listView/"
            f"{level1}/{level2}/{level3}/{level4}/?include"
            f"Participants=true&useCombined=true&lang=en_US&market=US",
            headers={"X-Corr-Id": self.user_correlation_id},
            name=f"Homepage<kambi>/offerings/ListView",
        ).json()["events"]
        ...
        self.client.get(
            f"{KAMBI_OFFERING_URL}/offering/v2018/{KAMBI_OFFERING_ID}/betoffer/event/{eventids}.json?market"
            f"=US&lang=en_US",
            headers={"X-Corr-Id": self.user_correlation_id},
            name=f"Homepage<kambi>/offerings/betoffer/event",
        )
    ...
    def get_balance(self, _, wait):
        with self.client.post(
            f"{WH_BASE_URL}/usergateway/getbalance",
            headers={
                "Consumer": "script",
    ...
    def place_bet(self, data, wait):
        ...
        bettor = Bettor(
            client=self.client,
            kambi_offering_url=KAMBI_OFFERING_URL,
            kambi_player_api=KAMBI_PLAYER_URL,
            kambi_offering_id=KAMBI_OFFERING_ID,
            kambi_session_id=self.kambi_session_id,
    ...
    def get_coupon_history(self, _, wait):
        with self.client.get(
            f"{KAMBI_PLAYER_URL}/player/api/v2019/{KAMBI_OFFERING_ID}/coupon/history.json",
            headers={
    ...
    def fetch_temp_homepage(self, _, wait):
        temp_url = 'https://platform.staging.casino-platform.example.com/dummy.html'
