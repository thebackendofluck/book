...
"""
Payments Platform — FastAPI entry point.
...
from psp.adyen import AdyenAdapter, AdyenSettings
...
def _build_registry() -> PSPRegistry:
    ...
    The chapter runtime keeps local execution self-contained by registering
    in-memory demo adapters here, while `psp/*.py` models the live provider
    contracts.
    """
    ...
    class StubAdapter(PSPAdapter):
        ...
        async def withdraw(self, withdrawal: Withdrawal) -> PSPResponse:
            return PSPResponse(
                success=True,
                external_transaction_id=f"WDR-{withdrawal.withdrawal_id[:8]}",
                status=PS.SUCCEEDED,
                raw_response={"stub": True},
            )

    registry = PSPRegistry()
    from psp.boleto import BoletoAdapter

    for name in ["adyen", "paypal", "braintree", "trustly", "pix", "neteller", "skrill"]:
        adapter = StubAdapter(name)
        adapter.supports_withdrawals = name in {"trustly", "neteller", "skrill"}
        registry.register(adapter)
    registry.register(BoletoAdapter())
    return registry
...
def _build_router(registry: PSPRegistry) -> PSPRouter:
    ...
    router.add_rule(RoutingRule(PaymentMethod.BOLETO, "BR", primary="boleto"))
    router.add_rule(RoutingRule(PaymentMethod.NETELLER, "*", primary="neteller"))
    router.add_rule(RoutingRule(PaymentMethod.SKRILL, "*", primary="skrill"))
...
# Used only to verify inbound Adyen webhook HMAC signatures before any
# deposit callback is allowed to mutate state. Fails closed when
# ADYEN_NOTIFICATION_HMAC_KEY is unset — see psp/adyen.py.
_adyen_webhook_verifier = AdyenAdapter(
    AdyenSettings(
        api_key="",
        merchant_account="",
        notification_hmac_key=os.getenv("ADYEN_NOTIFICATION_HMAC_KEY", ""),
    )
)
...
async def lifespan(app: FastAPI):
    log.info("payments_platform.starting")
    yield
    log.info("payments_platform.stopped")
...
_allowed_origins = [o.strip() for o in os.getenv("PAYMENTS_CORS_ALLOWED_ORIGINS", "").split(",") if o.strip()]
...
async def deposit_callback(payment_id: str, body: PSPCallbackRequest):
    ...
    if deposit.provider_info.provider_name == "adyen":
        # Never mutate deposit state on an unauthenticated webhook body:
        # verify the Adyen HMAC signature first. Fails closed if unconfigured.
        notification_items = [{"NotificationRequestItem": dict(body.raw_response)}]
        if not _adyen_webhook_verifier.verify_notification_hmac(notification_items):
...
def _admin_id_from_principal(principal: Principal) -> int:
    try:
        return int(principal.sub)
    except ValueError:
        raise HTTPException(status_code=400, detail="Authenticated principal has no numeric admin id")
...
async def approve_withdrawal(
    ...
    admin_user_id = _admin_id_from_principal(principal)
    try:
        w = await withdrawal_service.approve(withdrawal_id, admin_user_id)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    return _withdrawal_response(w)
...
async def reject_withdrawal(
    ...
    admin_user_id = _admin_id_from_principal(principal)
    try:
        w = await withdrawal_service.reject(withdrawal_id, admin_user_id, body.reason)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    return _withdrawal_response(w)
