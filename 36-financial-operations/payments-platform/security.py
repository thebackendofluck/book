...
sig = HMAC-SHA256(base64url(json_claims), PAYMENTS_JWT_SECRET). Claims are
...
SECRET_KEY: str = os.getenv("PAYMENTS_JWT_SECRET", "")

_bearer_scheme = HTTPBearer(auto_error=False)


class ServiceRole(str, Enum):
    SERVICE = "service"  # trusted internal caller (already authenticated the player upstream)
    ADMIN = "admin"       # backoffice operator
