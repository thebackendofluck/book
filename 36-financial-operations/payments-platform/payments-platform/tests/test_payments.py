...
# ---------------------------------------------------------------------------
# Multiple chapter-36 microservices ship their own `models.py` (at
# minimum: ledger-service, payments-platform, treasury-service). With
# vanilla sys.path the first to load wins `sys.modules["models"]` and
# any sibling with different classes (Deposit, FraudScore, ...) gets
# the wrong module. We sidestep the race by pre-loading the local
# copies explicitly via importlib.util.spec_from_file_location before
# the plain `from models import ...` runs.
...
# `models` first, then every sibling that does `from models import ...`
# at module load time.
# Evict any `psp*` package a sibling test file may have installed.
...
def _pin_local_modules():
    """Re-pin this service's modules so lazy model imports inside
    state_machine and lazy `from main import app` calls inside test
    bodies don't break when sibling test files overwrite sys.modules."""
    for _stale in [k for k in list(sys.modules) if k == "psp" or k.startswith("psp.")]:
        sys.modules.pop(_stale, None)
...
def test_fraud_block_blacklisted_user():
    ...
    store.is_blacklisted_user = lambda uid: True
...
def test_fraud_block_blacklisted_ip():
    store = InMemoryFraudStore()
    store.is_blacklisted_ip = lambda ip: True
    checker = FraudChecker(store)
    result = checker.evaluate(make_deposit())
    assert result.decision == FraudDecision.BLOCK
...
async def test_withdrawal_kyc_blocked():
    kyc = KycService()
    kyc.get_status = lambda uid: KycStatus.PENDING
