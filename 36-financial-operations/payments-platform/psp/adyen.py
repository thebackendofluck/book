...
class AdyenAdapter(PSPAdapter):
    ...
    def verify_notification_hmac(
        ...
        if not self.settings.notification_hmac_key:
            # Fail closed: without a configured key we cannot verify the
            # notification, so reject it. Never accept an unverifiable
            # payment webhook.
            logger.error("HMAC key not configured — rejecting notification")
            return False
