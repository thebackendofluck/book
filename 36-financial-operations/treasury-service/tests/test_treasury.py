...
_load_local_module("treasury", "treasury.py")
...
def make_settlement(**kwargs) -> Settlement:
    ...
    # Forward every key the caller supplied -- the previous version hard-
    # coded six fields and silently dropped optional overrides such as
    # `initiated_at`, which made the age_hours test always read zero.
...
# ---------------------------------------------------------------------------
# 19. Float management — adequate
# ---------------------------------------------------------------------------


def test_float_adequate():
    svc = fresh_service()
    svc.record_settlement("adyen", 200_000, "FLOAT-001")
    req = svc.check_float(total_player_liabilities=100_000)
    assert req.is_adequate is True
    assert req.float_ratio >= 2.0
    assert req.surplus_or_deficit == 100_000


# ---------------------------------------------------------------------------
# 20. Float management — deficit
# ---------------------------------------------------------------------------


def test_float_deficit():
    svc = fresh_service()
    # No settlements -> zero available
    req = svc.check_float(total_player_liabilities=500_000)
    assert req.is_adequate is False
    assert req.surplus_or_deficit < 0


# ---------------------------------------------------------------------------
# 21. Float management — zero liabilities
# ---------------------------------------------------------------------------


def test_float_zero_liabilities():
    svc = fresh_service()
    req = svc.check_float(total_player_liabilities=0)
    assert req.is_adequate is True


# ---------------------------------------------------------------------------
# 22. Float management — withdrawal coverage days
# ---------------------------------------------------------------------------


def test_float_coverage_days():
    svc = fresh_service()
    svc.record_settlement("adyen", 300_000, "COV-001")
    req = svc.check_float(
        total_player_liabilities=100_000,
        avg_daily_withdrawals=100_000,
    )
    # Available = 300k PSP clearing. Coverage = 300k / 100k = 3 days
    assert req.withdrawal_coverage_days == 3.0


# ---------------------------------------------------------------------------
# 23. Treasury report — daily
# ---------------------------------------------------------------------------


def test_daily_report_generation():
    svc = fresh_service()
    svc.record_settlement("adyen", 100_000, "RPT-001")
    svc.record_settlement("paypal", 50_000, "RPT-002", SettlementDirection.OUTBOUND)

    report = svc.generate_report(
        period_start=datetime.now(timezone.utc) - timedelta(days=1),
        period_end=datetime.now(timezone.utc),
        total_player_liabilities=80_000,
        report_type="daily",
    )
    assert report.report_type == "daily"
    assert report.total_inbound == 100_000
    assert report.total_outbound == 50_000
    assert report.net_flow == 50_000


# ---------------------------------------------------------------------------
# 24. Treasury report — alerts on float deficit
# ---------------------------------------------------------------------------


def test_report_alerts_on_float_deficit():
    svc = fresh_service()
    # No settlements -> zero available
    report = svc.generate_report(
        period_start=datetime.now(timezone.utc) - timedelta(days=1),
        period_end=datetime.now(timezone.utc),
        total_player_liabilities=1_000_000,
    )
    assert any("CRITICAL" in a for a in report.alerts)


# ---------------------------------------------------------------------------
# 25. Treasury report — stuck settlement alert
# ---------------------------------------------------------------------------


def test_report_alerts_on_stuck_settlements():
    import uuid as _uuid

    svc = fresh_service()
    s = svc.record_settlement("adyen", 10_000, "STUCK-RPT")
    aged = s.model_copy(update={
        "settlement_id": str(_uuid.uuid4()),
        "reference": "STUCK-RPT-AGED",
        "initiated_at": datetime.now(timezone.utc) - timedelta(hours=48),
    })
    svc._store.save_settlement(aged)
    ...
    assert report.stuck_settlement_count >= 1
