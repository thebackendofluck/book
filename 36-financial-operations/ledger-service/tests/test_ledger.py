...
# `reconciliation.py`. Install this service's copies explicitly
# before the plain `from models import ...` runs -- see the matching
# preamble in payments-platform/tests/test_payments.py for the full
# explanation.
...
def _pin_local_modules():
    """Re-pin this service's modules at module-scoped fixture time so
    any sibling test file that ran afterward during collection and
    stomped on sys.modules can't affect our test bodies."""
    _load_local_module("models", "models.py")
    _load_local_module("ledger", "ledger.py")
    _load_local_module("event_translator", "event_translator.py")
    _load_local_module("reconciliation", "reconciliation.py")
...
@pytest.mark.asyncio
async def test_trial_balance_on_clean_data(translator, ledger):
    """Trial balance must show debits == credits."""
    await translator.deposit("p1", 5000, "stripe")
    await translator.bet("p1", 2000, "roulette")
    ...
    assert trial.imbalance == 0
    assert trial.entry_count == 4  # 2 postings x 2 entries each


@pytest.mark.asyncio
async def test_trial_balance_empty_ledger(ledger):
    """Empty ledger has balanced trial balance."""
    trial = await ledger.trial_balance()
    assert trial.is_balanced is True
    assert trial.total_debits == 0
    assert trial.total_credits == 0


@pytest.mark.asyncio
async def test_trial_balance_detects_corruption(ledger):
    """Injecting an orphan should unbalance the trial balance."""
    ...
    trial = await ledger.trial_balance()
    assert trial.is_balanced is False
    assert trial.imbalance == 500
...
@pytest.mark.asyncio
async def test_audit_trail_created_on_posting(translator, ledger):
    """Every posting should create an audit trail entry."""
    await translator.deposit("p1", 5000, "stripe")
    trail = await ledger.get_audit_trail()
    assert len(trail) == 1
    assert trail[0].action == "posting_created"


@pytest.mark.asyncio
async def test_audit_trail_preserves_metadata(translator, ledger):
    """Audit trail should include the event metadata."""
    await translator.deposit("p1", 5000, "stripe")
    trail = await ledger.get_audit_trail()
    assert trail[0].metadata["event"] == "deposit"
    assert trail[0].metadata["player_id"] == "p1"


@pytest.mark.asyncio
async def test_audit_trail_filter_by_group(translator, ledger):
    """Filter audit trail by entry_group_id."""
    posting1 = await translator.deposit("p1", 5000, "stripe")
    await translator.bet("p1", 2000, "slots")

    trail = await ledger.get_audit_trail(posting1.entry_group_id)
    assert len(trail) == 1
    assert trail[0].entry_group_id == posting1.entry_group_id


# --- 13. Fee and suspense event types ---


@pytest.mark.asyncio
async def test_psp_fee_creates_correct_entries(translator, ledger):
    """PSP fee: DEBIT FEE_EXPENSE, CREDIT PSP_CLEARING."""
    posting = await translator.psp_fee("stripe", 250, "processing")
    entries = posting.entries

    debit = next(e for e in entries if e.direction == Direction.DEBIT)
    credit = next(e for e in entries if e.direction == Direction.CREDIT)

    assert "FEE_EXPENSE" in debit.account_id
    assert "PSP_CLEARING" in credit.account_id
    assert debit.amount == credit.amount == 250


@pytest.mark.asyncio
async def test_suspense_park_and_release(translator, ledger):
    """Suspense: park money, then release it back."""
    # First deposit to create a player balance
    await translator.deposit("p1", 10_000, "stripe")

    # Park 3000 in suspense
    await translator.suspense_park("PLAYER_WALLET:p1", 3_000, "fraud_review")

    # Check suspense has balance
    suspense_bal = await ledger.get_account_balance("SUSPENSE:fraud_review")
    assert suspense_bal.total_debits == 3_000

    # Release back
    await translator.suspense_release("PLAYER_WALLET:p1", 3_000, "cleared")
