...
class InMemoryLedgerStore:
    ...
    async def append_audit(self, audit: AuditTrailEntry) -> None:
        async with self._lock:
    ...
    async def get_audit_trail(
        ...
        if entry_group_id:
            return [a for a in self._audit_trail if a.entry_group_id == entry_group_id]
...
class Ledger:
    ...
    async def create_posting(
        ...
        reason = meta.get("reason", meta.get("event", "posting"))
        ...
        await self.store.append_audit(audit)
    ...
    async def trial_balance(self) -> TrialBalance:
        """
        Compute the trial balance across ALL accounts.

        In double-entry bookkeeping, the sum of all debits must equal
        the sum of all credits. If not, the books are corrupt.

        This is the fundamental check that auditors run and that
        NJ DGE / MGA / UKGC expect to pass at any point in time.
        """
        all_entries = await self.store.get_all_entries()

        total_debits = sum(e.amount for e in all_entries if e.direction == Direction.DEBIT)
        total_credits = sum(e.amount for e in all_entries if e.direction == Direction.CREDIT)
    ...
    async def get_audit_trail(
        ...
        """Return audit trail entries, optionally filtered by posting group."""
        return await self.store.get_audit_trail(entry_group_id)
