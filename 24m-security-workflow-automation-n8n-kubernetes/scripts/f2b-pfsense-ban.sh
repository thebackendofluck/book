...
# f2b-pfsense-ban.sh — add an IP to the pfSense fail2ban alias via the
# REST API v2 (read-modify-PATCH cycle + X-API-Key auth). See Chapter 24m 13.3.
set -euo pipefail
...
echo "$TS [BAN] jail=$JAIL ip=$IP id=$ID patch=$PATCH apply=$APPLY" >>"$LOG"
