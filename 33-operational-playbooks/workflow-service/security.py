...
sig = HMAC-SHA256(base64url(json_claims), WORKFLOW_JWT_SECRET). Claims are
only trusted after the signature is recomputed and compared in constant time
and the expiry is checked. Mirrors the RBAC dependency pattern in this
chapter's backoffice-admin/security/access_control.py
(Depends(require_roles(...))), adapted to a stdlib HMAC token so this service
...
SECRET_KEY: str = os.getenv("WORKFLOW_JWT_SECRET", "")

_bearer_scheme = HTTPBearer(auto_error=False)


class ServiceRole(str, Enum):
    SERVICE = "service"  # trusted internal caller (KYC, withdrawal, dispute services)
    ADMIN = "admin"       # backoffice reviewer/approver
