...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'kyc_app_user') THEN
...
-- Note: audit_log uses BIGSERIAL PK (not partitioned).
-- To partition by timestamp in production, audit_log would need PARTITION BY RANGE (timestamp)
-- and the PK/UNIQUE constraints must include the partition column.
