-- =============================================================================
-- FILE NOTE: This UPPERCASE file (VNJDGE09PII.sql) uses Oracle naming conventions
...
-- =============================================================================
-- NJ DGE View 09: PERSONALLY IDENTIFIABLE INFORMATION (PII)
...
-- This file is an alias/symlink-equivalent for the view defined in dge_09_pii_data.sql.
--
-- Installation note: The canonical view definition lives in dge_09_pii_data.sql.
-- This file re-creates the same view under the DGE-spec view name vNJDGE09PII
-- to satisfy the DGE filename/view naming convention documented in the spec.
-- Both files create identical views.
--
-- Sources:
...
--
-- Access control: view is owned by dge schema, readable by dge_readonly_external only.
-- The DGE regulator connects via IP-whitelisted VPN with SSL/TLS.
--
-- Reference: Chapter 2 — Regulation and Compliance Landscape / NJ DGE Reporting
...
-- Performance note: the full-table range queries issued by DGE during
-- reporting windows should use the LAST_UPDATE_TIME_EASTERN_INDEX column
-- (timestamptz, not varchar), which is covered by:
--   idx_ifa_changed_on     ON user_information_field_audit(changed_on)
--   idx_uks_user_id_updated_on  ON user_kyc_status(user_id, updated_on DESC)
-- These indexes are created in dge_indexes.sql.
