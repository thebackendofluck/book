...
Designed for iGaming CI/CD pipelines where leaked credentials could
expose payment processing systems, player databases, or game provider
API integrations. Combines pattern matching with entropy-based scoring
to reduce false positives while catching real secrets.
"""
...
class SimpleMLSecretDetector:
    ...
    def predict(self, text):
        ...
        is_secret = score >= 5
        confidence = max(0.0, min(score / 13.0, 1.0))
...
def load_default_training_data():
    """Load default training data with example patterns."""
    secret_samples = [
...
def main():
    ...
    if args.json:
        ...
        if result["confidence"] >= args.threshold:
            print("HIGH PROBABILITY - Potential secret detected!")
            sys.exit(1)
        else:
