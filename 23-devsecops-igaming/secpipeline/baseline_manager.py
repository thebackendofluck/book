...
In iGaming environments, maintaining a secrets baseline prevents
false positives from blocking deployments while ensuring new secrets
are caught. This tool manages the lifecycle of that baseline.
"""
    ...
    def validate_baseline(self):
        ...
        try:
            if not self.baseline_file.is_file():
                print(f"Baseline path is not a file: {self.baseline_file}")
                return False
            ...
            if baseline["version"] not in ["0.1", "0.2", "1.0"]:
                print(f"Unknown baseline version: {baseline['version']}")
