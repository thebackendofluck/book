...
resource "aws_eip" "nat" {
  ...
  tags = merge(
    var.tags,
    {
      Name = "${var.project_name}-${var.environment}-nat-eip${var.single_nat_gateway ? "" : "-${count.index + 1}"}"
    }
  )
}
...
resource "aws_nat_gateway" "main" {
  ...
  tags = merge(
    var.tags,
    {
      Name = "${var.project_name}-${var.environment}-nat${var.single_nat_gateway ? "" : "-${count.index + 1}"}"
    }
  )
...
resource "aws_route_table" "private" {
  ...
  tags = merge(
    var.tags,
    {
      Name = "${var.project_name}-${var.environment}-private-rt${var.enable_nat_gateway ? "-${count.index + 1}" : ""}"
    }
  )
}
...
# VPC Endpoints for AWS Services (for private access)
