...
# monitor_tde_performance.sh - Monitor PostgreSQL TDE performance metrics
# Tracks encryption/decryption performance and key retrieval latency
...
# Get PostgreSQL TDE metrics
get_postgres_metrics() {
    # In real implementation, query PostgreSQL for TDE metrics
    cat << 'EOF'
{
  "connections": 45,
  "active_queries": 12,
...
get_hsm_metrics() {
    # In real implementation, query HSM for performance metrics
    cat << 'EOF'
{
  "key_retrievals": 1250,
...
calculate_performance() {
    local postgres_data
    postgres_data=$(get_postgres_metrics)
    local hsm_data
    
    # Extract values
    hsm_data=$(get_hsm_metrics)
    local connections
    connections=$(echo "$postgres_data" | jq -r '.connections')
    local tde_ops
    tde_ops=$(echo "$postgres_data" | jq -r '.tde_operations')
    local avg_query
    avg_query=$(echo "$postgres_data" | jq -r '.avg_query_time')
    local key_retrievals
    key_retrievals=$(echo "$hsm_data" | jq -r '.key_retrievals')
    local avg_retrieval
    avg_retrieval=$(echo "$hsm_data" | jq -r '.avg_retrieval_time')
    local failed_ops
    
    # Calculate rates
    failed_ops=$(echo "$hsm_data" | jq -r '.failed_retrievals')
    local tde_ops_per_sec
    tde_ops_per_sec=$(echo "scale=2; $tde_ops / $MONITOR_INTERVAL" | bc 2>/dev/null || echo "0")
    local key_ops_per_sec
    
    # Calculate efficiency
    key_ops_per_sec=$(echo "scale=2; $key_retrievals / $MONITOR_INTERVAL" | bc 2>/dev/null || echo "0")
    local efficiency
    efficiency=$(echo "scale=3; $tde_ops / ($key_retrievals + 1)" | bc 2>/dev/null || echo "0")
...
check_thresholds() {
    ...
    avg_query=$(echo "$metrics" | jq -r '.avg_query_time_ms')
    local avg_retrieval
    avg_retrieval=$(echo "$metrics" | jq -r '.avg_key_retrieval_ms')
    local failed_ops
    failed_ops=$(echo "$metrics" | jq -r '.failed_operations')
    
    local alerts=""
    
    # Check query time (should be < 100ms for good performance)
    if (( $(echo "$avg_query > 100" | bc -l 2>/dev/null || echo "0") )); then
        alerts="${alerts}WARNING: High query time (${avg_query}ms)\n"
    fi
    
    # Check key retrieval time (should be < 50ms)
...
display_metrics() {
    ...
    echo "═══════════════════════════════════════════════════════════════"
    echo "              PostgreSQL TDE Performance Monitor"
    echo "═══════════════════════════════════════════════════════════════"
    echo ""
